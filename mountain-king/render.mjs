...
//   node render.mjs full [fps] [crf]  -> ./out/mountain-king.mp4
...
await page.goto('http://localhost:4180/index.html?render=1', { waitUntil: 'load' });
await page.evaluate(() => document.fonts.ready);
const info = await page.evaluate(() => window.MV.load('audio/mountain-king.mp3'));
...
if (mode === 'contact') {
  ...
  const mp4 = path.join(out, 'mountain-king.mp4');
  const r = spawnSync(ffmpeg, ['-y', '-loglevel', 'error', '-framerate', String(FPS), '-i', path.join(frames, 'f%05d.jpg'), '-i', (fs.existsSync(path.join(root, 'audio', 'mountain-king.wav')) ? path.join(root, 'audio', 'mountain-king.wav') : path.join(root, 'audio', 'mountain-king.mp3')), '-c:v', 'libx264', '-preset', 'medium', '-crf', String(CRF), '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-shortest', '-movflags', '+faststart', mp4], { encoding: 'utf8' });
